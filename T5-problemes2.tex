...
\draw
  (6,-3) node[or gate, inputs=nnn] (or) {}
...
\draw
  ...
  (and2.input 1 -| 0.0,0)          node[contact] {}
  (and2.input 2 -| 1.2,0)          node[contact] {}

  (and3.input 1 -| 1.2,0)          node[contact] {}
  (and3.input 2 -| 2.4,0) ++(.5,0) node[contact] {}
;
...
\node at (4,-2) [and gate, inputs=nn] (and1) {};
\node at (4.8,-2) [not gate US, draw] (andn1) {};
\draw (and1.output) -- (andn1.input);
\foreach \a in {1,...,2}
  \draw (and1.input \a -| -0.5,0) -- (and1.input \a);

\node at (4,-3) [and gate, inputs=nn] (and2) {};
\node at (4.8,-3) [not gate US, draw] (andn2) {};
\draw (and2.output) -- (andn2.input);
\foreach \a in {1,...,2}
  \draw (and2.input \a -| -0.5,0) -- (and2.input \a);

\node at (4,-4) [and gate, inputs=nn] (and3) {};
\node at (4.8,-4) [not gate US, draw] (andn3) {};
\draw (and3.output) -- (andn3.input);
\foreach \a in {1,...,2}
  \draw (and3.input \a -| -0.5,0) -- (and3.input \a);
...
\draw
  (6,-3) node[and gate, inputs=nnn] (or) {}
...
\draw
  ...
  (and1.input 2 -| 1.2,0)          node[contact] {}
...
Una \textsf{XOR} de N entrades es comporta com una \textsf{OR} de N entrades, sempre que no hi hagi més d'una entrada que sigui 1. En l'expressió donada...
...
(0.0,0) coordinate (d) node[anchor=south] {$a$} (d) -- ++(0,-4.7)
(d) ++(.5, -1) node[xor gate, inputs=nn, point down] (d_not) {}
(d) ++(0,-0.3) node[contact] {} (d) ++(0,-0.3) -| (d_not.input 2)
...
\node at (4,-4) [and gate, inputs=nnn] (and3) {};
\foreach \a in {1,...,3}
  \draw (and3.input \a -| -0.5,0) -- (and3.input \a);

\draw
  (5.3,-3) node[xor gate, inputs=nnn] (or) {}
...
\draw
  ...
  (and3.input 3 -| 2.4,0)          node[contact] {}
;
...
(d) ++(.5, -1) node[nand gate, inputs=nn, point down] (d_not) {}
(d) ++(0,-0.3) node[contact] {} (d) ++(0,-0.3) -| (d_not.input 1)
(d) ++(0,-0.45) node[contact] {} (d) ++(0,-0.45) -| (d_not.input 2)
...
\node at (4,-2) [nand gate, inputs=nn] (and1) {};
\foreach \a in {1,...,2}
  \draw (and1.input \a -| -0.5,0) -- (and1.input \a);

\node at (4,-3) [nand gate, inputs=nn] (and2) {};
\foreach \a in {1,...,2}
  \draw (and2.input \a -| -0.5,0) -- (and2.input \a);

\node at (4,-4) [nand gate, inputs=nn] (and3) {};
\foreach \a in {1,...,2}
  \draw (and3.input \a -| -0.5,0) -- (and3.input \a);

\node at (4,-5) [nand gate, inputs=nnn] (and4) {};
\foreach \a in {1,...,3}
  \draw (and4.input \a -| -0.5,0) -- (and4.input \a);

\draw
  (5.5,-3.5) node[nand gate, inputs=nnnn] (or) {}
  (or.input 1) -- ++(-0.2,0) |- (and1.output)
...
\draw
  ...
  (and4.input 1 -| 0.0,0)          node[contact] {}
  (and4.input 2 -| 1.2,0) ++(.5,0) node[contact] {}
  (and4.input 3 -| 2.4,0) ++(.5,0) node[contact] {}
;
...
\begin{center} \begin{tikzpicture}[circuit logic US, circuit ee IEC] \draw
  ...
  (1.0,0.75) node[xor gate] (xor1) {}
  ($ (2.3,0.75)!.5!(2.3,0) $) node[xor gate] (xor2) {}
